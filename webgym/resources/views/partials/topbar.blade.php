<header class="bg-white border-b border-gray-200" x-data="{ open: false, hasNewNotification: false }">
    <div class="flex justify-between items-center p-4">
        ...
        <div class="flex items-center space-x-4">
            ...
            @php
                // Mặc định tìm kiếm chung 
                $searchAction = '#'; 
                $searchPlaceholder = "Tìm kiếm ...";
                $searchValue = request('search');

                // Nếu đang ở trang Store
                if (request()->routeIs('admin.store.*')) {
                    $searchAction = route('admin.store.index'); 
                    $searchPlaceholder = "Tìm tên sản phẩm, ID, ...";
                }
                ...
                // Nếu đang ở trang Khách hàng
                elseif (request()->routeIs('admin.customers.*')) {
                    $searchAction = route('admin.customers.index');
                    $searchPlaceholder = "Tìm tên khách hàng, email, ...";
                }
                // Nếu đang ở trang Huấn luyện viên
                elseif (request()->routeIs('admin.trainers.*')) {
                    $searchAction = route('admin.trainers.index');
                    $searchPlaceholder = "Tìm tên huấn luyện viên, chuyên môn, ...";
                }
                // Nếu đang ở trang Blogs
                elseif (request()->routeIs('admin.blogs.*')) {
                    $searchAction = route('admin.blogs.index');
                    $searchPlaceholder = "Tìm tiêu đề, tác giả, ...";
                }
                // Nếu đang ở trang Gói tập 
                elseif (request()->routeIs('admin.packages.*')) {
                    $searchAction = route('admin.packages.index');
                    $searchPlaceholder = "Tìm tên gói, ...";
                }
                // Nếu đang ở trang Lớp học
                elseif (request()->routeIs('admin.classes.*')) {
                    $searchAction = route('admin.classes.index');
                ...
                elseif (request()->routeIs('admin.payments.*')) {
                    ...
                    $searchPlaceholder = "Tìm mã giao dịch, tên khách hàng, ...";
                }
                // Nếu đang ở trang khuyến mãi
                elseif (request()->routeIs('admin.promotions.*')) {
                    $searchAction = route('admin.promotions.index');
                    $searchPlaceholder = "Tìm mã khuyến mãi, tên khuyến mãi, ...";
                }
            @endphp
            ...
            <!-- Nút cài đặt, thông báo, avatar người dùng -->
