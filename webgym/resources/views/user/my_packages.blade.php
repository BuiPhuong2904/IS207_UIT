...
<div x-data="{ currentTab: 'all' }">
    ...
    {{-- KIỂM TRA: NẾU KHÔNG CÓ GÓI TẬP NÀO --}}
    @if(collect($packages)->isEmpty())
        ...
        <div class="flex flex-col items-center justify-center py-16 bg-white rounded-2xl shadow-sm border border-gray-100 text-center">
            ...
            <h3 class="text-lg font-bold text-black mb-2 font-montserrat">Bạn chưa sở hữu gói tập nào</h3>
            <p class="text-[#333333]/80 mb-6 mx-auto font-open-sans ">Đăng ký gói tập ngay hôm nay để bắt đầu hành trình thay đổi vóc dáng cùng GRYND!</p>
            
            <a href="#" class="px-6 py-2.5 bg-[#1976D2] hover:bg-[#0D47A1]/90 text-white font-semibold rounded-full transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5 font-open-sans">
                Xem các gói tập
            </a>
        </div>

    @else
        {{-- NẾU CÓ DỮ LIỆU: HIỆN BỘ LỌC VÀ DANH SÁCH --}}   
        ...
        <div x-show="currentTab === 'active' && {{ collect($packages)->where('status', 'active')->count() }} === 0" class="text-center text-gray-500 py-10 font-open-sans" style="display: none;">
            Bạn chưa có gói tập nào đang hoạt động.
        </div>
        <div x-show="currentTab === 'expired' && {{ collect($packages)->where('status', 'expired')->count() }} === 0" class="text-center text-gray-500 py-10 font-open-sans" style="display: none;">
